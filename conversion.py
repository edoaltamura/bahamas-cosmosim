...
def comoving_density(header, density):
    scale_factor = 1 / (header.data.Redshift + 1)
    return np.multiply(density, header.data.HubbleParam ** 2 * scale_factor ** -3)


def comoving_length(header, coord):
    scale_factor = 1 / (header.data.Redshift + 1)
    return np.multiply(coord, scale_factor / header.data.HubbleParam)
...
def comoving_mass(header, mass):
    return np.divide(mass, header.data.HubbleParam)


def comoving_kinetic_energy(header, kinetic_energy):
    scale_factor = 1 / (header.data.Redshift + 1)
    _kinetic_energy = np.multiply(kinetic_energy, scale_factor)
    _kinetic_energy = np.divide(_kinetic_energy, header.data.HubbleParam)
    return _kinetic_energy


def comoving_momentum(header, mom):
    scale_factor = 1 / (header.data.Redshift + 1)
    return np.multiply(mom, np.sqrt(scale_factor) / header.data.HubbleParam)


def comoving_ang_momentum(header, angmom):
    scale_factor = 1 / (header.data.Redshift + 1)
    return np.multiply(angmom, np.sqrt(scale_factor ** 3) / np.power(header.data.HubbleParam, 2.))
